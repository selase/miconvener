...
- [x] **Stage 7: Responsive Navigation & Offline PWA**: Web app manifest scoped to `/my/` via `PlatformAttendeeAccessController::manifest()`, service worker (`public/attendee-sw.js` and `registerServiceWorker.js`) precaching shell and branding assets while never caching authenticated API responses in HTTP cache, opt-in "Save ticket offline" snapshot store in `localStorage` (`offlineTicketStore.js`) with 7-day post-event expiry, explicit omission of sensitive attendee fields (email, phone, certificates, notes), automatic offline storage purge on sign-out, responsive safe-area-aware sticky bottom navigation on mobile (<768px, 44px touch targets for Overview, Ticket, My day, More) and desktop header navigation (>=768px), offline banner, and "Saved offline" pill with remove action. Verified with 2 passing tests in `PlatformAttendeePwaTest.php` (11 assertions), 55 passing platform tests (329 assertions), 0 PHPStan errors, clean Pint, and clean Vite build.
- [x] **Stage 8: Contextual Actions**:
  - [x] **8.1 Durable Service Requests**: Status progression endpoint (`GET /service-requests`), duplicate active request prevention (`open`, `acknowledged`, `in_progress`), urgent medical priority escalation, and live progression stepper UI in `GetHelpPanel.jsx`.
  - [x] **8.2 Contextual Live Polls & Quizzes**: Live poll endpoints with server-derived opaque respondent tokens (`hash_hmac`), quiz scoring, instant results, and `PollPanel.jsx`.
  - [x] **8.3 Event Q&A / Forum**: Forum index & post endpoints accessible to confirmed attendees of private events, anonymous posting, upvoting with opaque derived tokens, host reply badging, and `ForumPanel.jsx`.
  - [x] **8.4 Dynamic Forms & Surveys**: Schema-driven forms auto-bound to verified registration identity, `requires_check_in` eligibility enforcement, dynamic group sync, and `FormsPanel.jsx`.
  - [x] **8.5 Portal Navigation Integration**: Lifecycle-aware tabs, active poll badging, urgent help requests count badging, and responsive desktop/mobile navigation in `Portal.jsx`.
  - [x] **8.6 Verification & Self-Review**: Hardened with backend duplicate form submission rejection (422), live poll total_votes and percentage computation on retrieval, forum attachment MIME whitelist validation, and expanded field support (`rating`, `multiselect`, `date`, `boolean`) in `FormsPanel.jsx`. Verified with 11 passing tests in `PlatformAttendeeContextualActionsTest.php` (67 assertions), 66 passing feature tests across all 9 platform attendee suites (396 assertions), 0 PHPStan errors, clean Pint, and clean Vite build.
- [x] **Stage 9: Record Panels (Certificates, Abstracts, and Attendance)**:
  - [x] **9.1 Controller Preloading & Workspace Integration**: Preload `initialCertificates`, `initialAbstracts`, and `initialAttendance` in `PlatformAttendeeAccessController::page()` avoiding data waterfalls; query and surface event-specific certificate and attendance records in `PlatformAttendeeWorkspaceController::show()`.
  - [x] **9.2 Certificates Panel (`CertificatesSection.jsx`)**: Responsive credential cards with role badges, CPD hours badge (`cpd_hours > 0`), separate "Verify" (`verification_url`, `target="_blank"`, `rel="noopener noreferrer"`) and "Download PDF" (`download_url`) actions, deduplication across registration and direct recipient email (`unique('uuid')`), and speaker/volunteer certificate support.
  - [x] **9.3 Abstracts Panel (`AbstractsSection.jsx`)**: Submissions and co-authorships list with human-readable status badges, presentation preferences, abstract code badge, deduplication across multi-author records, and support for co-authors without event registrations.
  - [x] **9.4 Attendance Panel (`AttendanceSection.jsx`)**: Strictly separate "Sessions attended" and "Certified hours" sub-sections; strict enforcement of invariant: NO manufactured sum or total of attendance hours or dwell-derived CPD hours; clear accreditation explanatory note.
  - [x] **9.5 Conditional Navigation & State (`MyPortal.jsx`)**: Conditional record tabs (`Events`, `Certificates`, `Abstracts`, `Attendance`) rendered only when items exist, active tab persistence in URL search params (`?tab=`), parallel client-side data fetch (`Promise.all`), fallback notice on Events tab when records exist elsewhere, and immediate state purge on sign-out.
  - [x] **9.6 Verification**: 10 passing feature tests in `tests/Feature/Events/PlatformAttendeeRecordsTest.php` (89 assertions), full suite of 94 passing platform attendee tests (547 assertions), 0 PHPStan errors, clean Pint, and clean Vite build (3.10s).
- [x] **Stage 10: Material Release Policy**:
  - [x] **10.1 Schema & Migrations**: Added `speaker_slide_policy` (`before`, `during`, `after` default `after`) to `events` table via `2026_09_24_100000_add_speaker_slide_policy_to_events_table.php` and `provenance` (`organizer`, `speaker` default `organizer`) to `event_materials` table via `2026_09_24_100100_add_provenance_to_event_materials_table.php`; backfilled existing speaker slide materials from `event_speakers.slides_material_id`.
  - [x] **10.2 Domain Models & Speaker Uploads**: Updated `Event` with `SPEAKER_POLICY_*` constants, fillable attributes, and casts; updated `EventMaterial` with `PROVENANCE_*` constants, `eventSpeaker` relationship, `isSpeakerDeck()`, `isOrganizerMaterial()`, and delegated `isReleased()` to `MaterialReleasePolicy`; set provenance explicitly on speaker portal uploads (`SpeakerPortalController`) and tenant organiser uploads (`EventMaterialController`).
  - [x] **10.3 Core `MaterialReleasePolicy` Service**: Implemented `MaterialReleasePolicy` as single source of truth for material release logic: explicit `release_at` always takes precedence; undated organiser materials release immediately; speaker decks follow event `speaker_slide_policy` (`before` unlocks immediately even if sessionless, `during` unlocks at first session `min(starts_at)`, `after` unlocks at last session `max(ends_at)`, and sessionless decks stay withheld under during/after); `releasedMaterialsForSession()` gathers session organiser files and linked speaker decks, deduplicating co-presenter decks within the session.
...
- [x] **End-to-End Chrome Browser QA Verification**:
  - Verified live in Google Chrome against Herd domain `https://miconvener.test/my` and `https://miconvener.test/my/events/01a0d21a-abaf-73fa-9102-1d3ad8e8476e`.
  - **Sign-in & Verification Challenge**: Confirmed 6-digit access code entry (`123456`), session authentication, rate limiter integration, and immediate transition to Attendee Dashboard.
  - **Action-First Dashboard Hierarchy**: Verified `Needs Attention` (unpaid tickets + pending ticket transfers), `Live Now` (Africa Tech Summit with VIP seat assignment A-12 and live badges), `Organisers / Upcoming Events`, and collapsible `Past Events`.
