...
function DeliverableList({ event, sponsor, onChange }) {
    ...
    const addDeliverable = async (e) => {
        ...
        setSaving(true);
        await csrfFetch(
            route('tenant.events.sponsors.deliverables.store', {
                event: event.id,
                sponsor: sponsor.id,
            }),
            {
    ...
    const toggle = async (deliverable) => {
        await csrfFetch(
            route('tenant.events.sponsors.deliverables.update', {
                event: event.id,
                sponsor: sponsor.id,
    ...
    const remove = async (deliverable) => {
        await csrfFetch(
            route('tenant.events.sponsors.deliverables.destroy', {
                event: event.id,
                sponsor: sponsor.id,
                deliverable: deliverable.id,
            }),
                ...
                <ul className="mb-3 divide-y divide-border">
                    {sponsor.deliverables.map((d) => (
                        <li key={d.id} className="flex items-center gap-2.5 py-2">
                            ...
                            />
                            <span
                                className={`flex-1 text-[13px] ${d.is_done ? 'text-ink-tertiary line-through' : 'text-ink'}`}
                            >
                                {d.description}
                            </span>
                            <button
...
function SponsorCard({ event, sponsor, onChange }) {
    ...
    const remove = async () => {
        await csrfFetch(
            route('tenant.events.sponsors.destroy', { event: event.id, sponsor: sponsor.id }),
            { method: 'DELETE' }
        );
        onChange();
    };
            ...
            <div className="flex items-center gap-3 p-3">
                ...
                <div className="min-w-0 flex-1">
                    ...
                    <div className="text-xs text-ink-secondary">
                        {sponsor.booth ? `Booth ${sponsor.booth}` : 'No booth assigned'}
                    </div>
                    {total > 0 && (
        ...
        <div className="max-w-4xl">
            <div className="mb-5 flex items-center justify-between">
                <p className="text-sm text-ink-secondary">
                    Who is paying, what you promised them, and what is still outstanding.
                </p>
                <a
            ...
            <div className="mb-6 grid grid-cols-1 gap-px bg-border sm:grid-cols-3">
                ...
                <div className="bg-surface p-4">
                    ...
                    <div className="mt-1 text-xl font-semibold text-accent">
                        {formatMoney(income, event.currency)}
                    </div>
                </div>
                <div className="bg-surface p-4">
                    <div className="text-xs text-ink-secondary">Promises kept</div>
                    <div className="mt-1 text-xl font-semibold text-ink">
                        {doneCount} of {allDeliverables.length}
                    </div>
                </div>
            </div>
            ...
            {TIERS.map((tier) => {
                ...
                return (
                    <div key={tier} className="mb-5">
                        <b className="mb-2 block text-sm font-medium text-ink">
                            {TIER_LABEL[tier]}
                        </b>
            ...
            {sponsors.length === 0 && (
                <p className="mb-5 text-sm text-ink-secondary">No sponsors yet.</p>
            )}
