...
function buildGrid(room) {
    ...
    return rowLetters.map((letter) => ({
        letter,
        seats: Array.from(
            { length: room.seats_per_row },
            (_, i) => `${letter}-${String(i + 1).padStart(2, '0')}`
        ),
    }));
}
...
function NewRoomForm({ event, onChange }) {
    ...
    return (
        <form
            onSubmit={submit}
            className="grid grid-cols-[1fr_120px_120px_auto] items-end gap-3 border border-border p-4"
        >
            <Input
                label="Room name"
...
function SeatAssignPicker({ event, room, seatLabel, onDone }) {
    ...
    useEffect(() => {
        ...
        const t = setTimeout(async () => {
            const res = await csrfFetch(
                `${route('tenant.events.venue.unseated', { event: event.id })}?q=${encodeURIComponent(query)}`
            );
    ...
    const assign = async (registrationId) => {
        await csrfFetch(
            route('tenant.events.venue.seats.assign', { event: event.id, room: room.id }),
            {
                method: 'POST',
...
export default function VenuePanel({ event, venueRooms }) {
    ...
    const unassign = async (assignment) => {
        await csrfFetch(
            route('tenant.events.venue.seats.unassign', {
                event: event.id,
                room: room.id,
    ...
    const confirmDeleteRoom = async () => {
        ...
        await csrfFetch(
            route('tenant.events.venue.rooms.destroy', { event: event.id, room: deleting.id }),
            { method: 'DELETE' }
        );
    ...
    return (
        <div className="max-w-3xl">
                    ...
                    >
                        ...
                        >
                            ...
                            <span className="text-[11px] text-ink-secondary">
                                {r.assignments.length}/{r.rows * r.seats_per_row} seated
                            </span>
                        </button>
                        <button
            ...
            {room && (
                <div className="grid gap-5 lg:grid-cols-[1fr_280px]">
                    <div className="border border-border p-6">
                        <div className="mx-auto mb-6 w-fit border border-border px-8 py-1.5 text-center text-[11px] uppercase tracking-wide text-ink-secondary">
                            Stage
                        </div>
                        <div className="flex flex-col items-center gap-1.5 overflow-x-auto">
                            {grid.map((row) => (
                                <div key={row.letter} className="flex items-center gap-1.5">
                                    <span className="w-4 font-mono text-[10px] text-ink-secondary">
                                        {row.letter}
                                    </span>
                                    {row.seats.map((label) => {
                        ...
                        <div className="mt-6 flex justify-center gap-5 text-[12px] text-ink-secondary">
                            <span className="flex items-center gap-1.5">
                                <span className="h-3 w-3 border border-border" /> Free
                            </span>
                            <span className="flex items-center gap-1.5">
                    ...
                    <div>
                        {!picked && (
                            <p className="text-sm text-ink-secondary">
                                Click a seat to assign or view who's in it.
                            </p>
                        )}
                        ...
                        {picked?.assignment && (
                            <div className="border border-border p-4">
                                ...
                                <div className="mt-2 text-[13.5px] text-ink">
                                    {picked.assignment.registration_name}
                                </div>
                                <Button
                                    onClick={() => unassign(picked.assignment)}
                                    className="mt-3"
                                >
                                    Unassign
                                </Button>
            ...
            <ConfirmModal
                ...
                description={
                    deleting
                        ? `"${deleting.name}" and its ${deleting.assignments.length} seat assignment(s) will be permanently removed.`
                        : ''
                }
