...
export default function FormsPanel({ event }) {
    ...
    const loadData = async () => {
        ...
        try {
            const res = await csrfFetch(
                route('tenant.events.dynamic-forms.index', { event: event.id })
            );
            if (res.ok) {
    ...
    const handleSaveForm = async (e) => {
        ...
        try {
            const url = editingForm
                ? route('tenant.events.dynamic-forms.update', {
                      event: event.id,
                      form: editingForm.id,
                  })
            ...
            const payload = {
                ...
                submission_limit: formData.submission_limit
                    ? parseInt(formData.submission_limit, 10)
                    : null,
    ...
    const handleDeleteForm = async () => {
        ...
        try {
            const res = await csrfFetch(
                route('tenant.events.dynamic-forms.destroy', {
                    event: event.id,
    ...
    const handleDuplicate = async (f) => {
        try {
            const res = await csrfFetch(
                route('tenant.events.dynamic-forms.duplicate', {
                    event: event.id,
                    form: f.id,
                }),
                { method: 'POST' }
    ...
    const openSubmissions = async (f) => {
        ...
        try {
            const res = await csrfFetch(
                route('tenant.events.dynamic-forms.submissions', {
                    event: event.id,
                    form: f.id,
                })
            );
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                    ...
                    <p className="text-sm text-slate-500 dark:text-slate-400">
                        Create general-purpose surveys, CME evaluations, workshop signups, and
                        dietary questionnaires on the fly.
                    </p>
                </div>
            ...
            {loading ? (
                ...
                <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-12 text-center space-y-3">
                    ...
                    <p className="text-xs text-slate-500 max-w-md mx-auto">
                        Create custom in-event surveys, CME evaluations, workshop selection forms,
                        or feedback polls with custom schemas.
                    </p>
                    <Button variant="primary" onClick={openCreateModal}>
                        <Plus className="w-4 h-4 mr-1.5" />
                        Create First Form
                    ...
                    <div className="grid grid-cols-2 gap-3 pt-2 border-t border-slate-100 dark:border-slate-800">
                        ...
                        <label className="flex items-center gap-2 text-xs font-medium text-slate-700 dark:text-slate-300 cursor-pointer">
                            <Checkbox
                                ...
                                onChange={(checked) =>
                                    setFormData({ ...formData, requires_check_in: checked })
                                }
                            />
            ...
            >
                <div className="space-y-4 max-h-[75vh] flex flex-col">
                    ...
                    <div className="flex justify-end pt-2">
                        <Button onClick={() => setSubmissionsModalOpen(false)}>Close</Button>
                    </div>
                </div>
            </Modal>
