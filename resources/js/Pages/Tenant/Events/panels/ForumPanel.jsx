...
function ReplyForm({ event, thread, onDone }) {
    ...
    const submit = async (e) => {
        ...
        setSaving(true);
        await csrfFetch(
            route('tenant.events.forum.reply', { event: event.id, thread: thread.id }),
            {
                method: 'POST',
...
function ThreadCard({ event, thread, onChange }) {
    ...
    const moderate = async (patch) => {
        await csrfFetch(
            route('tenant.events.forum.moderate', { event: event.id, thread: thread.id }),
            {
                method: 'PATCH',
    ...
    const remove = async () => {
        await csrfFetch(
            route('tenant.events.forum.destroy', { event: event.id, thread: thread.id }),
            { method: 'DELETE' }
        );
        onChange();
    };
    ...
    const ban = async () => {
        const reason = window.prompt(
            `Ban ${thread.author_email ?? 'this author'} from posting in this forum? Reason (optional):`
        );
        if (reason === null) return;
        const response = await csrfFetch(
            route('tenant.events.forum.ban', { event: event.id, thread: thread.id }),
            {
                method: 'PATCH',
    ...
    return (
        <div
            className={`border p-4 ${thread.reports_count > 0 ? 'border-danger-fg/40 bg-danger-bg/40' : 'border-border'}`}
        >
            <div className="flex items-start justify-between gap-3">
                <div>
                    <div className="flex flex-wrap items-center gap-2">
                        ...
                        {thread.is_pinned && (
                            <Pin className="h-3.5 w-3.5 text-accent" strokeWidth={1.75} />
                        )}
                        ...
                        {thread.reports_count > 0 && (
                            <StatusPill status="failed">
                                {thread.reports_count} report{thread.reports_count > 1 ? 's' : ''}
                            </StatusPill>
                        )}
                    </div>
                    ...
                    <div className="mt-1.5 flex items-center gap-3 text-xs text-ink-tertiary">
                        <span>
                            {thread.author_name}
                            {thread.author_email ? ` · ${thread.author_email}` : ''}
                        </span>
                        <span className="flex items-center gap-1">
                            <ThumbsUp className="h-3 w-3" strokeWidth={1.75} /> {thread.votes_count}
                        </span>
                    </div>
                </div>
                <div className="flex shrink-0 gap-1.5">
                    ...
                    <button
                        onClick={() => moderate({ is_hidden: !thread.is_hidden })}
                        title="Hide"
                        className="text-ink-secondary hover:text-warning-fg"
                    >
                ...
                <ul className="mt-3 space-y-2 border-t border-border pt-3">
                    {thread.replies.map((r) => (
                        <li key={r.id} className="text-[13px]">
                            <div className="flex items-center gap-2">
                                <b className={r.is_from_host ? 'text-accent' : 'text-ink'}>
                                    {r.author_name}
                                </b>
                                {r.tag && (
                                    <StatusPill status={TAG_STATUS[r.tag]}>
                                        {TAG_LABEL[r.tag]}
                                    </StatusPill>
                                )}
                            </div>
...
function BannedAuthors({ event, bans, onChange }) {
    ...
    const unban = async (ban) => {
        await csrfFetch(
            route('tenant.events.forum.bans.destroy', { event: event.id, ban: ban.id }),
            { method: 'DELETE' }
        );
        onChange();
    };
    ...
    return (
        <div className="border border-border p-4">
            <b className="mb-2 flex items-center gap-1.5 text-sm font-medium text-ink">
                <Flag className="h-3.5 w-3.5" strokeWidth={1.75} /> Banned from this forum
            </b>
            <ul className="divide-y divide-border">
                {bans.map((b) => (
                    <li key={b.id} className="flex items-center justify-between py-2 text-[13px]">
                        ...
                        <button
                            onClick={() => unban(b)}
                            className="text-xs text-ink-secondary underline hover:text-accent"
                        >
                            Unban
                        </button>
                    </li>
    ...
    return (
        <div className="max-w-3xl space-y-4">
            <p className="text-sm text-ink-secondary">
                Questions from the floor, answers from your team, all in one place.
            </p>
            ...
            {threads.length > 0 ? (
                threads.map((t) => (
                    <ThreadCard key={t.id} event={event} thread={t} onChange={load} />
                ))
            ) : (
                <p className="text-sm text-ink-secondary">
                    No questions yet. They'll show up here once attendees post from the event page.
                </p>
            )}
        </div>
    );
